...
App **不内置任何频道数据**，播放列表全部来自运行时接口，两条来源解析后合并下发：

| 来源 | 说明 |
| --- | --- |
| 默认源服务 | `Constants.defaultIptvSourceApi` 只是默认值，用户可在「设置 → 直播源 → 默认源服务」里替换成自己的接口；**清空即停用内置源**，此时只加载自定义源 |
| 自定义直播源 | 用户在设置里填自己的订阅链接（支持 m3u / TVBox 格式） |

合并逻辑见 `IptvUtil.buildMergedGroups`（去重、分组、索引重排）。

**PR 请守住这条边界：**不要把具体频道或源地址写进代码与 assets，也不要把默认接口之外的第三方源地址塞进常量
——这既是版权风险的隔离，也让"换源"不需要改代码、不需要发版。
